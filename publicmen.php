...
<section class="banner-bottom-wthreelayouts py-lg-5 py-3">
	<div class="container-fluid">
		<div class="inner-sec-shop px-lg-4 px-3">
			<h3 class="tittle-w3layouts my-lg-4 mt-3">Men Collections </h3>
			<div class="row">
				<!-- product left -->
    ...
    <!-- Sorting options -->
    <label>
        <input type="checkbox" name="sort_price" <?php if(isset($_POST['sort_price'])) echo 'checked="checked"'; ?> />
        <span>Sort by Price (Low to High)</span>
    </label>
    <label>
        <input type="checkbox" name="sort_price_desc" <?php if(isset($_POST['sort_price_desc'])) echo 'checked="checked"'; ?> />
...
<!-- Add the following script inside the <script> tag -->
<script>
    $(document).ready(function () {
        // Event listener for sorting checkboxes
        $("input[name='sort_price']").on("change", function () {
            // Submit the form when the checkbox is changed
            $("#filter-form").submit();
        });

        $("input[name='sort_price_desc']").on("change", function () {
            // Submit the form when the checkbox is changed
            $("#filter-form").submit();
        });
    });
...
<div class="row">
        ...
        if (isset($_POST['submit'])) {
            $search = $_POST['search'];
            $q = "SELECT * FROM `product` WHERE category='men' AND product LIKE '%$search%'";
        } elseif (isset($_GET['product'])) {
            $pdt = $_GET['product'];
            $q = "SELECT * FROM `product` WHERE category='men' AND product = '$pdt'";
        } else {
            $q = "SELECT * FROM `product` WHERE category='men'";
        }
